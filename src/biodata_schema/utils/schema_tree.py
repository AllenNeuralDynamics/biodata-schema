...
from biodata_schema.base import DataCoreModel, _GenericModel

for _mod in core.__loader__.get_resource_reader().contents():
    if "__" not in _mod and _mod.endswith(".py"):
        importlib.import_module(f"biodata_schema.core.{_mod.replace('.py', '')}")
...
def _extract_expandable_types(annotation) -> list:
    ...
    if (
        ...
        and annotation.__module__.startswith("biodata_schema.")
    ):
        return [annotation]
...
def _extract_named_types(annotation) -> list:
    ...
    biodata_schema or biodata_models, for documentation-link purposes.
    ...
    if (
        ...
        if module.startswith("biodata_schema.") or module.startswith("biodata_models."):
            return [annotation]
...
def _doc_url(model_cls) -> str:
    ...
    ``biodata_schema.utils.docs.model_generator`` (core files get a top-level page,
    ...
    for prefix in ("biodata_schema.", "biodata_models."):
        ...
        if module.startswith(prefix):
            ...
            if prefix == "biodata_schema." and parts[0] == "core":
                doc_path = parts[1]
