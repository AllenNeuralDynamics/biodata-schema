...
`biodata-schema` is intentionally unopinionated about how images, arrays, time series, and data frames should be represented in files. Because it decouples metadata representation from data representation, `biodata-schema` is able to integrate with other formats in the field easily. We designed `biodata-schema` to specifically align well with Neurodata Without Borders (NWB), OME-NGFF, and the Brain Imaging Data Structure (BIDS).
...
`biodata-schema` describes significantly more metadata. We are actively developing an NWB extension ([ndx-aind-metadata](https://github.com/AllenNeuralDynamics/ndx-aind-metadata)) that can directly package biodata-schema metadata into NWB.
...
`biodata-schema` describes significantly more metadata. We plan to develop a utility for packaging biodata-schema metadata into OME-NGFF files, setting common fields as appropriate.
...
The primary reason we cannot use BIDS directly is that the file organization is inherently mutable. As new acquisitions are produced, many metadata files must change as they describe the entire group. biodata-schema is scoped specifically to a single data acquisition.
...
`biodata-schema` contains fields that align with array field names (for example, the name of a fiber implanted in a certain location is in procedures.json, and that name will also show up in photometry readout files), but `biodata-schema` avoids describing the outer and inner layouts of files and focuses instead on broader experimental metadata.

## Hugging Face

[Hugging Face](https://huggingface.co/) is a platform for hosting and running machine learning models. It has a schema for model metadata that is used to facilitate model discovery and visualization on their web page. The `biodata-schema` model schema extends the core concepts there to cover more detailed descriptions of model architecture, training conditions, and more.